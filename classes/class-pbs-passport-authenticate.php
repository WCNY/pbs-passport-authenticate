...
class PBS_Passport_Authenticate {
  ...
  public function setup_rewrite_rules() {
    add_rewrite_rule( 'pbsoauth/(authenticate|callback|loginform|activate|userinfo|alreadyamember)/?.*$', 'index.php?pbsoauth=$matches[1]', 'top');
  }
  ...
  public function rewrite_templates($template) {
    ...
    if ( get_query_var('pbsoauth')=='alreadyamember' )  {
      $template = trailingslashit($this->dir) . 'templates/alreadyamember.php';
    }
    return $template;
  }
